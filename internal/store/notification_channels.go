...
// Template 是出站 webhook 的自定义请求体模板（Go text/template），空为默认信封。
...
func (s *SQLNotificationChannelStore) List(ctx context.Context) ([]NotificationChannelRecord, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, type, name, url, secret, template, enabled, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Type, &c.Name, &c.URL, &c.Secret, &c.Template, &c.Enabled, &c.CreatedAt, &c.UpdatedAt); err != nil {
...
func (s *SQLNotificationChannelStore) Get(ctx context.Context, id string) (NotificationChannelRecord, error) {
	...
	err := s.db.QueryRowContext(ctx,
		...
		&c.ID, &c.Type, &c.Name, &c.URL, &c.Secret, &c.Template, &c.Enabled, &c.CreatedAt, &c.UpdatedAt)
...
func (s *SQLNotificationChannelStore) Upsert(ctx context.Context, ch NotificationChannelRecord) error {
	...
	if isSQLiteDriver(s.db) {
		_, err := s.db.ExecContext(ctx,
			`INSERT INTO notification_channels (id, type, name, url, secret, template, enabled, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
			 ON CONFLICT(id) DO UPDATE SET type=excluded.type, name=excluded.name, url=excluded.url,
			   secret=excluded.secret, template=excluded.template, enabled=excluded.enabled, updated_at=excluded.updated_at`,
			ch.ID, ch.Type, ch.Name, ch.URL, ch.Secret, ch.Template, ch.Enabled, ch.CreatedAt, ch.UpdatedAt)
		return err
	}
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO notification_channels (id, type, name, url, secret, template, enabled, created_at, updated_at)
		 ...
		 ON DUPLICATE KEY UPDATE type=VALUES(type), name=VALUES(name), url=VALUES(url),
		   secret=VALUES(secret), template=VALUES(template), enabled=VALUES(enabled), updated_at=VALUES(updated_at)`,
		ch.ID, ch.Type, ch.Name, ch.URL, ch.Secret, ch.Template, ch.Enabled, ch.CreatedAt, ch.UpdatedAt)
	return err
}
