...
var sqliteColumnBackfills = []struct {
	...
	// 通知通道自定义请求体模板（mirrors migrations/026）。
	{"notification_channels", "template", "TEXT NOT NULL DEFAULT ''"},
}
...
var sqliteMigrations = []string{
	...
	`CREATE UNIQUE INDEX IF NOT EXISTS capability_registry_name_idx ON capability_registry (name)`,
	`CREATE INDEX IF NOT EXISTS capability_registry_domain_status_idx ON capability_registry (domain, status)`,
	`CREATE TABLE IF NOT EXISTS capability_versions (
		...
		backend_adapter TEXT NOT NULL DEFAULT '',
	...
	`CREATE INDEX IF NOT EXISTS capability_versions_status_idx ON capability_versions (status)`,
	...
	`CREATE INDEX IF NOT EXISTS capability_usage_stats_capability_idx ON capability_usage_stats (capability_id)`,
